 ...
 * Social sign-in buttons for OAuth providers.
 */
...
interface SSOButtonsProps extends HTMLAttributes<HTMLDivElement> {
  /**
   * Available SSO providers to display.
   */
  providers?: SSOProvider[];
  /**
   * Handler when a provider button is clicked.
   */
  ...
  disabled?: boolean;
  /**
   * Loading state for a specific provider.
   */
...
const SSOButtons = forwardRef<HTMLDivElement, SSOButtonsProps>(
  (
    {
      className,
      ...
      <div ref={ref} className={cn('space-y-2', className)} {...props}>
        {providers.map((provider) => {
          ...
          return (
            ...
            >
              {isLoading ? (
                <span className="mr-2 h-4 w-4 motion-safe:animate-spin">...</span>
              ) : (

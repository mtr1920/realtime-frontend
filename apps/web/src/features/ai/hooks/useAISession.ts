...
import type { AISessionState, AISessionInfo, AIError, AITurn } from '../types/session.types';
import type { UseAISessionOptions, UseAISessionReturn } from '../types/hooks.types';
...
export function useAISession(options: UseAISessionOptions): UseAISessionReturn {
  const { participantId, audioDeviceId, inputMode = 'push_to_talk', callbacks } = options;
  ...
  const audioCapture = useAudioCapture({
    ...
    onAudioChunk: useCallback(
      (chunk: ArrayBuffer) => {
        ...
        send('ai.input.audio.append', { audio, turnId: turnIdRef.current ?? undefined });
      },
  ...
  useAISubscriptions(
    { setSessionInfo, setState, setCurrentTurn, setIsAISpeaking, setIsUserSpeaking, setError, clearSessionStartTimeout, clearTurnTimeout },
    { state, inputMode, isAISpeaking, currentTurn, callbacksRef, turnIdRef, audioPlayback }
  );

  // Session actions
  const actions = useAISessionActions(
    { state, inputMode, isConnected, isAISpeaking, participantId, audioDeviceId, stateRef, callbacksRef, turnIdRef, sessionStartTimeoutRef, turnTimeoutRef, audioCapture, audioPlayback, send },
    { setState, setCurrentTurn, setIsAISpeaking, setIsUserSpeaking, setError }
  );

  // Network disconnection handling
  useEffect(() => {
    if (previousConnectionState.current && !isConnected && state !== 'idle' && state !== 'error' && state !== 'ending') {
      const disconnectError: AIError = { code: 'DISCONNECTED', message: 'Connection lost during AI session', recoverable: true };
  ...
  useEffect(() => {
    return () => {
      ...
      clearAllTimeouts();
      if (isConnected) send('ai.session.end', { reason: 'user_action' });
    };
  }, [audioCapture, audioPlayback, isConnected, send, clearAllTimeouts]);
